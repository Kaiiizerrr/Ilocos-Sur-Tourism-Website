...
try {
    ...
    if (isset($segments[0]) && $segments[0] === 'auth') {
        ...
        // POST /api/auth/forgot-password  -> generate & send a 6-digit code.
        if ($segments === ['auth', 'forgot-password'] && $method === 'POST') {
            $payload = read_json_body();
            $email   = trim((string)($payload['email'] ?? ''));
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                json_response(['errors' => ['email' => 'Please enter a valid email address.']], 422);
            }
            // Only create/send a code when the account actually exists, but
            ...
            if ($db->emailExists($email)) {
                ...
                $db->createPasswordReset($email, $code, RESET_CODE_TTL_SECONDS);
                send_reset_email($email, $code);
                if (MAIL_DEV_MODE) {
                    $devCode = $code; // localhost demo aid only
                }
            }
            $resp = ['message' => 'If that email is registered, a verification code has been sent.'];
            if ($devCode !== null) {
        ...
        // POST /api/auth/reset-password  -> verify the emailed code, then set
        // the new password. Requires a valid, unexpired, single-use code.
        if ($segments === ['auth', 'reset-password'] && $method === 'POST') {
            ...
            $result = $db->verifyResetCode($email, $code, RESET_MAX_ATTEMPTS);
            if ($result === 'expired') {
                json_error('That code is invalid or has expired. Please request a new one.', 400);
            }
            if ($result === 'invalid') {
                json_error('Incorrect verification code. Please try again.', 400);
            }
            // Code verified — safe to update the password.
            $db->resetPassword($email, $newPw);
