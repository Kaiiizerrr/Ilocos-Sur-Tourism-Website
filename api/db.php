...
class Database
{
    ...
    private function initSchema(): void
    {
        ...
        // The forgot-password flow emails a one-time 6-digit code. We store only
        // a SHA-256 HASH of the code (never the code itself), with an expiry, an
        // attempt counter, and a used flag — so a code is single-use, short-lived,
    ...
    /**
     * Create a new password-reset code for $email. Any earlier unused codes for
     * the same email are invalidated first, so only the latest code is valid.
     * Only the SHA-256 hash of the code is stored.
     */
    public function createPasswordReset(string $email, string $code, int $ttlSeconds): void
    {
        $stmt = $this->pdo->prepare(
        ...
        $stmt = $this->pdo->prepare(
            'INSERT INTO password_resets (email, code_hash, expires_at, attempts, used, created_at)
             VALUES (:email, :code_hash, :expires_at, 0, 0, :created_at)'
        );
        $stmt->execute([
            'email'      => $email,
    ...
    /**
     * Verify a submitted code for $email. Returns one of:
     *   'ok'      — code correct; it is marked used (single-use).
     *   'invalid' — wrong code; the attempt counter is incremented.
     *   'expired' — no active code, it has expired, or too many wrong attempts.
     */
    public function verifyResetCode(string $email, string $code, int $maxAttempts): string
    {
        $stmt = $this->pdo->prepare(
        ...
        if ((int)$row['expires_at'] < time() || (int)$row['attempts'] >= $maxAttempts) {
            $this->pdo->prepare('UPDATE password_resets SET used = 1 WHERE id = ?')
                      ->execute([$row['id']]);
            return 'expired';
        }
        if (!hash_equals((string)$row['code_hash'], hash('sha256', $code))) {
            $this->pdo->prepare('UPDATE password_resets SET attempts = attempts + 1 WHERE id = ?')
                      ->execute([$row['id']]);
            return 'invalid';
        }
        $this->pdo->prepare('UPDATE password_resets SET used = 1 WHERE id = ?')
